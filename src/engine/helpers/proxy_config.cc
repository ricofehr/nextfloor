...
void ProxyConfig::ManageProgramParameters(int argc, char *argv[])
{
    ...
    while (cnt < argc) {
        ...
        if (arg == "-s") {
            const std::string arg2(argv[cnt++]);
            ProxyConfig::setSetting("load_objects_freq", libconfig::Setting::TypeFloat, std::stof(arg2));
        }

        /* Vsync */
        if (arg == "-v") {
            const std::string arg2(argv[cnt++]);
            ProxyConfig::setSetting("vsync", libconfig::Setting::TypeBoolean, std::stoi(arg2) == 1);
        }

...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindCollisionNeighbors(const glm::vec3& coords) const
{
    ...
    std::vector<nextfloor::mesh::Mesh*> bottom_neighbors, top_neighbors;
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);

    tbb::task_group tasks;
    std::vector<nextfloor::mesh::Mesh*> center_neighbors, right_neighbors, left_neighbors;
    std::vector<nextfloor::mesh::Mesh*> right_bottom_neighbors, right_top_neighbors;
    std::vector<nextfloor::mesh::Mesh*> left_bottom_neighbors, left_top_neighbors;
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontCenterPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontRightPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontRightBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontRightTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x - 1, coords.y + 1, coords.z - 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontLeftPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y, coords.z - 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontLeftBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y - 1, coords.z - 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontLeftTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y + 1, coords.z - 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindFrontTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x, coords.y + 1, coords.z - 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindRightPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);

    tbb::task_group tasks;
    std::vector<nextfloor::mesh::Mesh*> center_neighbors, bottom_neighbors, top_neighbors;
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindRightCenterPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindRightBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindRightTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x - 1, coords.y + 1, coords.z);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackCenterPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x, coords.y, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackRightPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x - 1, coords.y, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackRightBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x - 1, coords.y - 1, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackRightTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x - 1, coords.y + 1, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackLeftPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackLeftBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y - 1, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackLeftTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y + 1, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x, coords.y - 1, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBackTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x, coords.y + 1, coords.z + 1);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindLeftPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindLeftCenterPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y, coords.z);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindLeftBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y - 1, coords.z);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindLeftTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x + 1, coords.y + 1, coords.z);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindBottomPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindTopPositionCollisionNeighbors(const glm::vec3& coords) const
{
    std::vector<nextfloor::mesh::Mesh*> neighbors(0);
    auto neighbors_coord = glm::ivec3(coords.x, coords.y + 1, coords.z);
    if (IsCooordsAreCorrect(neighbors_coord) && IsPositionFilled(neighbors_coord)) {
...
std::vector<nextfloor::mesh::Mesh*> WiredGrid::FindOccupants(const glm::ivec3& coords) const
{
    return boxes_[coords.x][coords.y][coords.z]->occupants();
}

void WiredGrid::AddItem(nextfloor::mesh::Mesh* object)
{
    ParseGridForObjectPlacements(object);
}

void WiredGrid::ParseGridForObjectPlacements(nextfloor::mesh::Mesh* object)
{
    std::vector<nextfloor::mesh::GridBox*> coords_list;
    ...
    object->set_gridcoords(coords_list);
}
...
nextfloor::mesh::GridBox* WiredGrid::AddItemToGrid(const glm::ivec3& coords, nextfloor::mesh::Mesh* object)
{
    assert(IsCooordsAreCorrect(coords));
...
void WiredGrid::RemoveMesh(nextfloor::mesh::Mesh* object)
{
...
void WiredGrid::RemoveItemToGrid(const glm::ivec3& coords, nextfloor::mesh::Mesh* object)
{
    object->delete_gridcoord(boxes_[coords.x][coords.y][coords.z].get());

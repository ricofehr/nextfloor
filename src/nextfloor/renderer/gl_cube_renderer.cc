...
GlCubeRenderer::GlCubeRenderer(std::string texture)
:GlRenderer(texture)
{
   CreateVertexBuffer();
   CreateElementBuffer();
 ...
 *  Fill vertex buffer
 */
void GlCubeRenderer::CreateVertexBuffer() noexcept {
...
void GlCubeRenderer::CreateElementBuffer() noexcept {
    GLuint elements[] = {
        /* front */
        0, 1, 2,
        2, 3, 0,
 ...
 *  Fill texture buffer
 */
void GlCubeRenderer::CreateTextureBuffer() noexcept {
    int width, height;
    unsigned char* image;
...
void GlCubeRenderer::Draw(const glm::mat4& mvp) noexcept
{
    if (vertexbuffer_ == 0) {
        return;
    }
    ...
    glUniformMatrix4fv(GameWindow::getMatrixId(), 1, GL_FALSE, &mvp[0][0]);
